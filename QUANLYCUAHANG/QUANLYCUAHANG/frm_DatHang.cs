...
namespace QUANLYCUAHANG
{
    public partial class frm_DatHang : Form
    {
        ...
        {
            if (MessageBox.Show("Xác nhận đặt hàng?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
            {
                foreach (Control ctl in pan_DonDatHang.Controls)
                {
        ...
        private void pan_DonDatHang_Paint(object sender, PaintEventArgs e)
        {
            if (pan_DonDatHang.Controls.Count == 1)
            {
                dh.insertDonDatHang(frm_Main.nv.manv);
                Dondh.mahd = dh.layMaSPMoiNhat();
                btn_Luu.Visible = true;
        ...
        {
            DialogResult result = MessageBox.Show("Bạn chắc chắn muốn hủy đơn đặt hàng không?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private void frm_DatHang_MouseLeave(object sender, EventArgs e)
        {
            slt = soluong;
            tt = 0;
            soluong = 0;
